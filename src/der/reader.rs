...
use super::DerResult;
use super::DerErrorKind::{InvalidDer, InvalidTag, Eof};
...
impl<'a> DerReader<'a> {
    fn read_u8(&mut self) -> DerResult<u8> {
        if self.pos + 1 < self.pos {
            return der_err!(InvalidDer, "length overflow while reading DER");
        }
        ...
        match self.buf.get(self.pos) {
            ...
            None => return der_err!(InvalidDer, "out-of-range"),
        }
    }
    ...
    fn read_tag(&mut self) -> DerResult<Tag> {
        let (tag_class, is_constructed, tag) = {
            ...
            let tag = if b0 & 0b1_1111 == 0b1_1111 {
                ...
                return der_err!(InvalidDer, "unsupported tag value > 31 found");
            } else {
                b0 & 0b1_1111
            };
        ...
        let tag = if !is_constructed {
            ...
            if tag_class != TagClass::Universal {
                ...
                match tag {
                    0x00 => return der_err!(InvalidDer, "EndOfContents found in DER"),
                    ...
                    0x08 | 0x0b | 0x10 | 0x11 => {
                        return der_err!(InvalidDer, "Constructed tag found in Primitive: {}", tag);
                    }
                    _ => return der_err!(InvalidDer, "Unexpected tag in Primitive: {}", tag),
                }
            }
        } else {
            if tag_class != TagClass::Universal {
                ...
                match tag {
                    ...
                    0x00 ... 0x0c | 0x12 ... 0x1e => {
                        return der_err!(InvalidDer, "Primitive tag found in Constructed: {}", tag);
                    }

                    _ => return der_err!(InvalidDer, "Unexpected tag in Constructed: {}", tag),
                }
            }
        };
    ...
    fn read_len(&mut self) -> DerResult<usize> {
        let len = {
            let b0 = try!(self.read_u8());
            if b0 == 0b1000_0000 {
                return der_err!(InvalidDer, "indefinite length found in DER");
            } else if b0 >> 7 == 1 {
                // long form.
                let lenlen = b0 & 0b111_1111;
                if lenlen == 0b111_1111 {
                    return der_err!(InvalidDer, "illegal length");
                }
                // we just don't expect >= 2^16 bytes.
                if lenlen > 2 {
                    return der_err!(InvalidDer, "unsupported length: {}", lenlen);
                }
                let mut len = 0us;
                for _i in (0..lenlen) {
    ...
    fn read_value(&mut self, len: usize) -> DerResult<&'a [u8]> {
        ...
        if new_pos > self.buf.len() || new_pos < self.pos {
            return der_err!(InvalidDer, "length too large: {}", len);
        }
