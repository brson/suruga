        ...
        impl ::der::FromTlv for $ty_name {
            fn from_tlv(tag: ::der::Tag, value: &[u8]) -> ::der::DerResult<$ty_name> {
                match tag {
                    ...
                    _ => return der_err!(::der::DerErrorKind::InvalidTag,
                                         "unexpected tag: {:?}",
                                         tag),
                }
            }
...
macro_rules! ctx_sp {
    (P, $e:expr) => (
        ::der::Tag::Primitive($e, ::der::TagClass::ContextSpecific)
    );
    (C, $e:expr) => (
        ::der::Tag::Constructed($e, ::der::TagClass::ContextSpecific)
    );
}
    ...
    (
        ...
        impl ::der::FromTlv for $seq_name {
            fn from_tlv(tag: ::der::Tag, value: &[u8]) -> ::der::DerResult<$seq_name> {
                match tag {
                    ::der::Tag::Sequence => {
                        let seq_parser = ::der::reader::DerReader::new(value);
        ...
        impl $seq_name {
            fn from_seq(mut parser: ::der::reader::DerReader) -> ::der::DerResult<$seq_name> {
                ...
                while !parser.is_eof() {
                    let (tag, value) = try!(parser.next_tlv());
                    let item: $item_ty = try!(::der::FromTlv::from_tlv(tag, value));
                    seq.push(item);
                }
